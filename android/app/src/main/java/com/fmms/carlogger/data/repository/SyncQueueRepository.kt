...
class SyncQueueRepository(private val syncQueueDao: SyncQueueDao) {
    ...
    suspend fun enqueueFuelLog(log: com.fmms.carlogger.core.database.entity.FuelLogEntity) {
        ...
        val payload = JSONObject().apply {
            ...
            put("fuel_level_before_pct", num(log.fuelLevelBeforePct))
            put("fuel_liters_before", num(log.fuelLitersBefore))
            put("fuel_level_after_pct", num(log.fuelLevelAfterPct))
            put("fuel_liters_after", num(log.fuelLitersAfter))
            put("calculated_consumption_l100km", num(log.calculatedConsumptionL100km))
            put("prev_odometer_km", num(log.prevOdometerKm))
            put("fuel_consumed_liters", num(log.fuelConsumedLiters))
