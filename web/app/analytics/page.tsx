...
import { INITIAL_ASSETS, MOCK_EXPENSES, MOCK_FUEL_LOGS, MOCK_MAINTENANCE_RECORDS, MOCK_TRIPS } from '@/lib/data/mockData';
import { BarChart3, TrendingDown, TrendingUp, Car, DollarSign, Gauge } from 'lucide-react';

const fmt = (n: number) => n.toLocaleString('vi-VN');

export default function AnalyticsPage() {
  const totalFuelCost = MOCK_FUEL_LOGS.reduce((s, f) => s + f.total_cost, 0);
  const totalMaintCost = MOCK_MAINTENANCE_RECORDS.reduce((s, m) => s + m.cost, 0);
  const totalInsurance = MOCK_EXPENSES.filter(e => e.category === 'INSURANCE').reduce((s, e) => s + e.amount, 0);
  const totalExpenses = MOCK_EXPENSES.reduce((s, e) => s + e.amount, 0);
  const totalKm = INITIAL_ASSETS.reduce((s, a) => s + a.current_odometer_km, 0);
  const totalTripKm = MOCK_TRIPS.reduce((s, t) => s + t.distance_km, 0);
  const totalFleetValue = INITIAL_ASSETS.reduce((s, a) => s + a.current_value, 0);
  const totalPurchase = INITIAL_ASSETS.reduce((s, a) => s + a.purchase_price, 0);
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Tổng km đội xe', value: `${fmt(totalKm)} km`, sub: `${INITIAL_ASSETS.length} phương tiện`, color: 'var(--accent-cyan)', Icon: Gauge },
      ...
      <div className="p-5 rounded-2xl" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-default)' }}>
        ...
        <div className="flex items-end space-x-2 h-32">
          {[
            { m: 'T1', v: 1200 }, { m: 'T2', v: 980 }, { m: 'T3', v: 1450 },
            { m: 'T4', v: 850 }, { m: 'T5', v: 1630 }, { m: 'T6', v: 1350 },
            { m: 'T7', v: 2100 }, { m: 'T8', v: 1808 }, { m: 'T9', v: 0 },
            { m: 'T10', v: 0 }, { m: 'T11', v: 0 }, { m: 'T12', v: 0 },
          ].map((bar, i) => {
            const max = 2100;
            const h = bar.v > 0 ? Math.max(8, (bar.v / max) * 100) : 4;
            const isCurrentMonth = i === 7;
            return (
              <div key={i} className="flex-1 flex flex-col items-center space-y-1">
                <div className="w-full rounded-t-lg transition-all" style={{
                  height: `${h}%`,
                  background: bar.v === 0 ? 'var(--bg-hover)' : isCurrentMonth
                    ? 'linear-gradient(to top, var(--accent-cyan), #3B82F6)'
                    : 'linear-gradient(to top, rgba(56,189,248,0.4), rgba(59,130,246,0.4))',
                  minHeight: 4,
                }} />
                <span className="text-[9px]" style={{ color: isCurrentMonth ? 'var(--accent-cyan)' : 'var(--text-faint)' }}>{bar.m}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs text-center">
        {[
          { label: 'Tổng chuyến đi', value: MOCK_TRIPS.length, color: 'var(--accent-cyan)' },
          { label: 'Tổng km di chuyển', value: `${fmt(Math.round(totalTripKm))} km`, color: 'var(--text-primary)' },
          { label: 'TB tốc độ', value: `${(MOCK_TRIPS.reduce((s,t)=>s+t.average_speed_kmh,0)/MOCK_TRIPS.length).toFixed(1)} km/h`, color: 'var(--status-green)' },
          { label: 'TB tiêu thụ', value: '7.0 L/100km', color: 'var(--status-amber)' },
        ].map((s, i) => (
