...
export default function AssetDetailPage() {
  ...
  const handleDeleteAssetLoan = (loanId: string) => {
    setSecurityModal({
      ...
      description: 'CẢNH BÁO: Toàn bộ cấu hình khoản vay, bảng phân bổ các kỳ và các chi phí liên quan đến khoản vay của xe này sẽ bị xóa vĩnh viễn. Vui lòng nhập mã PIN Quản trị viên để tiếp tục.',
    ...
    const loadMasterCategories = async () => {
      try {
        ...
        if (!cancelled && list && list.length > 0) {
          setCategories(list.map((c: any) => typeof c === 'string' ? c : c.name));
          return;
        }
      } catch {}
      if (!cancelled) setCategories(DEFAULT_MAINT_CATEGORIES);
    };
  ...
  const handleDeleteOdoLog = (id: string) => {
    setSecurityModal({
      ...
      description: 'Xác nhận xóa mốc Odometer này khỏi lịch sử đo lường của xe. Vui lòng nhập mã PIN Quản trị viên để tiếp tục.',
  ...
  const handleDeleteExpense = (id: string) => {
    setSecurityModal({
      ...
      description: 'Xác nhận xóa vĩnh viễn khoản chi phí này khỏi sổ chi tiêu của xe. Vui lòng nhập mã PIN Quản trị viên để tiếp tục.',
  ...
  const handleDeleteMaint = (id: string) => {
    setSecurityModal({
      ...
      description: 'Xác nhận xóa bản ghi lịch sử bảo dưỡng này khỏi hồ sơ phương tiện. Vui lòng nhập mã PIN Quản trị viên để tiếp tục.',
  ...
  const handleDeleteFuel = (id: string) => {
    setSecurityModal({
      ...
      description: 'Xác nhận xóa lượt đổ xăng này khỏi lịch sử tiêu thụ nhiên liệu của xe. Vui lòng nhập mã PIN Quản trị viên để tiếp tục.',
  ...
  const handleDeletePart = (id: string) => {
    setSecurityModal({
      ...
      description: 'Xác nhận xóa phụ tùng / đồ chơi nâng cấp này khỏi danh sách trang bị của xe. Vui lòng nhập mã PIN Quản trị viên để tiếp tục.',
              ...
              {odoViewMode === 'monthly' && (
                <div className="space-y-5">
                  ...
                  {displayMonthlyReport.length > 0 && (
                    <div className="p-3.5 sm:p-4 rounded-2xl max-w-full overflow-hidden" style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-default)' }}>
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5 mb-3 min-w-0 max-w-full">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <p className="text-[11px] font-extrabold uppercase tracking-wider text-cyan-600 dark:text-cyan-400 truncate">
                            Biểu đồ Km &amp; Chi phí theo tháng {selectedOdoYear !== 'ALL' ? `(${selectedOdoYear})` : ''}
                          </p>
                        </div>

                        <div className="flex items-center justify-between md:justify-end flex-wrap gap-2 w-full md:w-auto min-w-0 max-w-full">
                          {/* Bộ lọc Năm */}
                          {availableOdoYears.length > 0 && (
                            <div className="flex items-center bg-slate-100 dark:bg-slate-800/90 p-0.5 rounded-xl border border-slate-200 dark:border-slate-700/80 max-w-full min-w-0 overflow-x-auto scrollbar-none">
                              <button
                                type="button"
                              ...
                              {availableOdoYears.map(yr => (
                                ...
                                >
                                  <span className="hidden sm:inline">Năm </span>{yr}
                                </button>
                              ))}
                            </div>
                          )}
        ...
        {activeTab === 'fuel' && (
          <div className="space-y-4">
            ...
            {assetMonthlyFuelData.length > 0 && (
              <div className="p-3.5 sm:p-4 rounded-2xl space-y-3 max-w-full overflow-hidden" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-default)' }}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5 min-w-0 max-w-full">
                  <div className="flex items-center space-x-2 min-w-0">
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center shadow-sm shrink-0" style={{ background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)' }}>
                      <Fuel className="w-4 h-4 text-amber-500" />
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-xs font-bold truncate" style={{ color: 'var(--text-primary)' }} title="Phân Tích Nhiên Liệu & Chi Phí Theo Tháng">
                    ...
                    <div className="flex items-center justify-between md:justify-end gap-2 flex-wrap w-full md:w-auto min-w-0 max-w-full">
                      <div className="flex items-center p-0.5 rounded-xl text-xs max-w-full min-w-0 overflow-x-auto scrollbar-none" style={{ background: 'var(--bg-hover)', border: '1px solid var(--border-subtle)' }}>
                        <button
        ...
        {activeTab === 'expenses' && (
          <div className="space-y-4">
            ...
            {displayedExpenses.length > 0 ? (() => {
              ...
              return (
                <div className="p-3.5 sm:p-5 rounded-2xl space-y-3 max-w-full overflow-hidden" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-default)' }}>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 min-w-0 max-w-full">
                    <div className="flex items-center space-x-2 min-w-0">
