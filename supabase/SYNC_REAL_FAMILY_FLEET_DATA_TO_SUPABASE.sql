-- ============================================================================
-- FMMS: ĐỒNG BỘ DỮ LIỆU XE & CHI PHÍ THỰC TẾ LÊN SUPABASE (XÓA BỎ DỮ LIỆU MẪU CŨ)
-- Chạy toàn bộ script này trong Supabase SQL Editor
-- ============================================================================

-- 1. Tắt RLS tạm thời để nạp dữ liệu
...
-- Bổ sung các cột mở rộng nếu chưa có
...
DECLARE
  ...
  target_user_id UUID;
BEGIN
  -- Lấy user bất kỳ trong auth.users làm owner mặc định
  SELECT id INTO target_user_id FROM auth.users LIMIT 1;

  -- UUID chuẩn hóa cố định cho từng xe thực tế
  ...
  -- Xóa sạch dữ liệu mẫu thử nghiệm cũ
  ...
  -- --------------------------------------------------------------------------
  -- 1. CHÈN 6 PHƯƠNG TIỆN THỰC TẾ CỦA GIA ĐÌNH
  -- --------------------------------------------------------------------------
...
-- Bật lại RLS với chính sách cấp quyền dùng chung cho toàn bộ gia đình
